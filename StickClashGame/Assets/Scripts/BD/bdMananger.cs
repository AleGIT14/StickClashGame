...
public class bdMananger : MonoBehaviour
{
    ...
    public void actualizarPuntos(string user, int puntosGanados)
    {

        string query1 = String.Format("UPDATE player_data SET actualPoints = actualPoints + @puntosGanados WHERE user_id IN (SELECT user_id FROM users WHERE username = @user);");
        string query2 = String.Format("UPDATE ranking SET totalPoints = totalPoints + @puntosGanados WHERE user_id IN (SELECT user_id FROM users WHERE username = @user);");
        ...
        using (conn = new MySqlConnection(connString))
        {
            ...
            try
            {
                ...
                cmd1.Parameters.AddWithValue("@user", user);
                cmd1.Parameters.AddWithValue("@puntosGanados", puntosGanados);
                cmd1.ExecuteNonQuery();

                MySqlCommand cmd2 = new MySqlCommand(query2, conn);

                cmd2.Parameters.AddWithValue("@user", user);
                cmd2.Parameters.AddWithValue("@puntosGanados", puntosGanados);
                cmd2.ExecuteNonQuery();
